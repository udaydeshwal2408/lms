        ...
        <div className="mx-auto box-content px-4 lg:w-[1260px] 2xl:relative ">
          <div className="mx-auto grid min-h-[450px] max-w-maxContentTab justify-items-cente py-8 lg:mx-0 lg:justify-items-start lg:py-0 xl:max-w-[810px]">
            ...
            <div className="relative block max-h-[30rem] lg:hidden">
              <Img src={thumbnail} alt="course thumbnail" className="aspect-auto w-full rounded-2xl" />
            ...
            <div className={`mb-5 flex flex-col justify-center gap-4 py-5 text-lg text-richblack-5`}>
              ...
              <div className="text-md flex flex-wrap items-center gap-2">
                ...
                {/* Fixed Pluralization for Reviews and Students */}
                <span>{`(${ratingAndReviews.length} ${ratingAndReviews.length === 1 ? "review" : "reviews"})`}</span>
                <span>{`${studentsEnrolled.length} ${studentsEnrolled.length === 1 ? "student" : "students"} enrolled`}</span>
              </div>
              <p className="capitalize "> Created By <span className="font-semibold underline">{instructor?.firstName || 'Unknown'} {instructor?.lastName || ''}</span></p>
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {tag && tag.map((item, ind) => (
                <p key={ind} className="bg-yellow-50 p-[2px] text-black rounded-full text-center font-semibold" >{item}</p>
              ))}
            </div>
          </div>
          ...
          <div className="max-w-[830px] mt-9">
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-wrap justify-between gap-2">
                <div className="flex gap-2">
                  {/* Fixed Pluralization for Sections and Lectures */}
                  <span>
                    {courseContent?.length || 0} {courseContent?.length === 1 ? "section" : "sections"}
                  </span>
                  <span>
                    {totalNoOfLectures} {totalNoOfLectures === 1 ? "lecture" : "lectures"}
                  </span>
            ...
            <div className="mb-12 py-4">
              ...
              <div className="flex items-center gap-4 py-4">
                <Img src={instructor?.image || 'https://api.dicebear.com/5.x/initials/svg?seed=Unknown'} alt="Author" className="h-14 w-14 rounded-full object-cover" />
                <div>
                  <p className="text-lg capitalize flex items-center gap-2 font-semibold">
                    {`${instructor?.firstName || 'Unknown'} ${instructor?.lastName || ''}`}
                    <span><MdOutlineVerified className='w-5 h-5 text-[#00BFFF]' /></span>
                  </p>
                  <p className="text-richblack-50">{instructor?.additionalDetails?.about || 'No information available'}</p>
                </div>
              </div>
            </div>
          </div>
