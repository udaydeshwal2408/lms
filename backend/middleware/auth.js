...
exports.auth = (req, res, next) => {
    try {
        // extract token from body, cookies or Authorization header
        let token = req.body?.token || req.cookies?.token || null;
        const authHeader = req.header('Authorization') || req.headers['authorization'] || null;
        if (!token && authHeader) {
            // handle formats like: "Bearer <token>" or just the token
        ...
        // quick format check before verification
